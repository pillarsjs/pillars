...
function routesWalker(gw, route, path){
  ...
  if (route.active && pathRegexp.test(path)) { // check partiality or entire path
    ...
    if ((isEndPath || haveChildren) && (!route.host || route.host == gw.host) && (route.port === undefined || route.port == gw.port) && (!route.method || route.method.concat(['HEAD','OPTIONS']).indexOf(gw.method) >= 0) && (route.https === undefined || route.https === gw.https)) {
