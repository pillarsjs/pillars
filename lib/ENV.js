JSON.decycle = function(val,deep){
	deep = deep || 4;
...
function decycleWalker(parents,path,val,deep){
	if(['undefined','number','boolean','string'].indexOf(typeof val)>=0 || val === null){
		...
		if(parents.indexOf(val) >= 0){
			...
			var copy,i,k,l;
			if(val.constructor.name.slice(-5)==='Array'){
...
JSON.stringifyDecycle = function(val,replacer,deep){
	return JSON.stringify(JSON.decycle(val,deep),replacer);
};
...
crypt.encrypt = function(data,password){
	...
	try {
		data = JSON.stringifyDecycle(data);
	} catch(error){
		return false;
	}
...
crypt.decrypt = function(data,password){
	...
	data = decipher.update(data.toString(),'hex','utf8');
