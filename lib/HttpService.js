...
function HttpService(config){
  var service = this;
  var ssl = config.key?true:false;
  var server = ssl?https.createServer(config):http.createServer();
  server.ssl = service.ssl = ssl;
  ...
  server
    ...
    //.on('timeout', function(socket) {socket.destroy();})
