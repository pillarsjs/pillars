...
var errorTemplate;
Object.defineProperty(Gangway,"errorTemplate",{
  enumerable : true,
  get : function(){return errorTemplate;},
  set : function(set){
    errorTemplate = set;
    templated.load(errorTemplate);
  }
});
Gangway.errorTemplate = paths.join(__dirname,'../templates/error.jade');
Gangway.maxZipSize = 2*1024*1024;
...
Gangway.prototype.file = function(file,clientname,download){
  ...
  function fileCheck(_stats){
    ...
    if(!gw.cacheck(gw.lastmod)) {
      stats = _stats;
      if(stats.size>Gangway.maxZipSize){gw.encoding='identity';}
...
Gangway.prototype.head = function(){
  ...
  gw.setHeader("Server", 'Pillars.js v'+pillarsPackage.version);
...
Gangway.prototype.render = function(template,locals){
  ...
  */
  ...
  var body = templated(template,locals,false,function(error,html){
    if(error){
      gw.error(500,error);
    } else {
      gw.send(body);
