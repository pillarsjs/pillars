...
function Gangway(server,req,res){
	...
	gw.routePath = gw.method+'+'+'http://'+gw.host+gw.path;
	...
	this.pathparams = function(){
		var matches = gw.pillar.regexps[gw.beam.id].exec(gw.routePath).slice(1);
		var repoint = matches.length-gw.beam.args.length;
		for(var i in gw.beam.args){
			gw.params[gw.beam.args[i]] = matches[parseInt(i)+repoint] || undefined;
		}
	}

	this.render = function(_locals){ 
		var locals = {
			title: gw.pillar.title+" - "+(_locals.h1 || 'unknow'),
			template: gw.beam.template,
			msgs:gw.msgs,
			pillar:gw.pillar,
			beam:gw.beam,
			fieldidr:fieldIdr,
		...
		var body = templates.render(gw.pillar.template,locals);
		gw.send(body);
	}
