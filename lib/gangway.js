...
function Gangway(server,req,res){
	...
	gw.t12n = function(text,params,domain){
		if(gw.language){
			var domain = domain || 'pillars';
			return gw.textualization.t12n(text,params,domain,gw.language);
		} else {
			return text;
		}
	}
	gw.t12nc = function(text,params,domain){
		if(gw.language){
			var domain = domain || 'pillars';
			return gw.textualization.t12nc(text,params,domain,gw.language);
		} else {
			return text;
		}
	}
	...
	function unlinktemp(file){
		fs.unlink(file, function(error){
			if(error){
				...
				console.log(gw.t12n('gangway.unlinktemp.error',{file:file}).red.inverse.white);
			} else {
				console.log(gw.t12n('gangway.unlinktemp.ok',{file:file}).red.inverse.white);
			}
		});
	}
}
	...
	if(sid && gw.session){
		...
		sessions.update({_id:sid},gw.session,function(error, result) {
			if(!error && result>0){
				...
				console.log(gw.t12n('gangway.session.save-error'));
			}
		});
	}
}
		...
		if(!sid){
			...
			sessions.findOne({_id:sid},function(error, result) {
				if(!error && result){
					...
					callback(new Error(gw.t12n('gangway.session.get-error')));
				}
			});
		}
	}
