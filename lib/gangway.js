...
var logger = Logger.pillars.addGroup('gangway');
...
function Gangway(req,res){
	...
	if(gw.cache.nonematch){
		try {
			var nonematch = JSON.parse(decrypt(gw.cache.nonematch));
			gw.cache.nonematch = nonematch;
		} catch(error){
			gw.cache.nonematch = false;
		}
	}

	if(gw.ranges.check){
		try {
			var check = JSON.parse(decrypt(gw.ranges.check));
			gw.ranges.check = check;
		} catch(error){
			gw.ranges.check = false;
		}
	}
...
Gangway.prototype.close = function(){
	...
	if(!gw.closed){
		...
		logger.info('close',{
			...
			code:gw.statusCode,
			size:gw.size,
			timer:parseInt((Date.now()-gw.timer)*100,10)/100
		});
		gw.emit('closed',gw);
	}
};
...
Gangway.prototype.file = function(file,clientname,download){
	...
	function fileCheck(_stats){
		gw.lastmod = _stats.mtime;
		if(!gw.cacheck(gw.lastmod)) {
			stats = _stats;
...
Gangway.prototype.cacheck = function(hash){
	...
	if(gw.cache.nonematch.hash && gw.cache.nonematch.hash === gw.etag.hash){
		gw.size = 0;
...
Gangway.prototype.head = function(){
	...
	gw.setHeader("Cache-Control", 'private, max-age='+maxage);
	gw.setHeader("Expires", (new Date(Date.now()+maxage*1000)).toUTCString());

	gw.setHeader("Last-Modified", (gw.lastmod || new Date()).toUTCString());

	var etag = encrypt(JSON.stringify(gw.etag));
	gw.setHeader("Etag", etag);
	//gw.setHeader("X-Etag", JSON.stringify(gw.etag));
	...
	//gw.setHeader("Transfer-Encoding","identity");
...
Gangway.prototype.send = function(data,type){
	...
	function end(body){
		gw.size = body.length;
		var hash = ENV.crypt.crypto.createHash('sha1').update(body).digest('hex');
		if(!gw.cacheck(hash)){
...
function getEncoding(encodings){
	...
	if(
		...
		(!all && encodings.deflate && encodings.deflate>0 && // not all valids but deflate is ok & best than gzip
			(
				!encodings.gzip || encodings.deflate>encodings.gzip
			)
		)
	){
		return 'deflate';
		...
		(!all && encodings.gzip && encodings.gzip>0 && // not all valids but gzip is ok & best than identity
			(
				!encodings.identity || encodings.gzip>encodings.identity
			)
		)
	){
		return 'gzip';
