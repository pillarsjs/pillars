...
Gangway.prototype.newSession = function(callback){
	...
	sessions.insert({timestamp:Date.now(),key:key},function(error, result) {
		if(!error && result[0]){
	...
	if(sid && gw.session){
		...
		sessions.update({_id:sid},gw.session,function(error, result) {
			if(!error && result>0){
				...
				console.log(gw.t12n('gangway.session.save-error'),error);
			}
		});
	}
}
		...
		if(!sid || !key){
			...
			sessions.findOne({_id:sid,key:key},function(error, result) {
				if(!error && result){
					...
					delete gw.cookie.sid;
					delete gw.cookie.key;
					gw.session = false;
...
/*  res.setHeader(
	...
	var gw = this;
	var cookiecontrol = ['sid','key'];
	var cookieset = [];
	for(var i in cookiecontrol){
		var cookievar = cookiecontrol[i];
...
Gangway.prototype.error = function(code,data){
	...
	if(util.isError(data)){
		body = templates.render('templates/error.jade',{
			title:explain,
			h1:h1,
