...
function Gangway(app,req,res){
	...
	gw.cookie = cookieParser((req.headers['cookie'] || ''));
	gw.cookieSet = [];
	gw.accessControl = {
		allowOrigin:false,
		allowMethods:false,
	...
	gw.t12n = function(text,params){return textualization.t12n(text,params,gw.language);}
...
Gangway.prototype.close = function(){
	...
	delete gw.app.server.gangways[gw.id];
}
...
Gangway.prototype.setCookie = function(name,value,config){
	...
	gw.cookieSet.push(cookieComposer(name,value,config));
}
...
Gangway.prototype.head = function(){
	...
	if(gw.accessControl.allowOrigin){
		gw.setHeader('Access-Control-Allow-Origin',gw.accessControl.allowOrigin);
	}
	if(gw.accessControl.allowMethods){
		gw.setHeader('Access-Control-Allow-Origin',gw.accessControl.allowMethods);
	}
	if(gw.accessControl.allowHeaders){
		gw.setHeader('Access-Control-Allow-Origin',gw.accessControl.allowHeaders);
	}
	
	if(gw.cookieSet.length>0){gw.setHeader("Set-Cookie", gw.cookieSet);}
	if(gw.encoding!='identity'){
	...
	gw.setHeader("X-Powered-By", 'Pillars-JS v'+PILLARS.version);
