...
function Gangway(req,res){
	...
	if(gw.cache.nonematch){
		try {
			var nonematch = JSON.parse(gw.cache.nonematch);
	...
	if(gw.ranges.check){
		try {
			var check = JSON.parse(gw.ranges.check);
...
Gangway.prototype.file = function(file,clientname,download){
	...
	function fileCheck(_stats){
		if(!gw.cacheck(_stats.mtime)) {
			stats = _stats;
			console.log(stats.size,'>',ENV.server.maxZipSize,stats.size>ENV.server.maxZipSize);
			if(stats.size>ENV.server.maxZipSize){gw.encoding='identity';}
			console.log(gw.cache.nonematch,gw.ranges,gw.encoding);
...
Gangway.prototype.cacheck = function(lastmod){
	...
	if(gw.cache.nonematch.lastmod && (new Date(gw.cache.nonematch.lastmod)).getTime() === gw.lastmod.getTime()){
...
Gangway.prototype.head = function(){
	...
	gw.setHeader("Cache-Control", 'private, max-age='+maxage); // no-cache
	gw.setHeader("Expires", (new Date(Date.now()+maxage*1000)).toUTCString());

	//gw.setHeader("Last-Modified", (gw.lastmod || new Date()).toUTCString());
	gw.etag.lastmod = (gw.lastmod || new Date()).getTime();
	var etag = JSON.stringify(gw.etag); // encrypt();
