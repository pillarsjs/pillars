...
Gangway.prototype.cleanTemp = function(){
	...
	function unlinktemp(file){
		fs.unlink(file, function(error){
			if(error){
				console.log(gw.t12n('gangway.unlinktemp.error',{file:file}));
			} else {
				console.log(gw.t12n('gangway.unlinktemp.ok',{file:file}));
			}
		});
	}
}
...
function queryHeap(query){
	...
	function merge(o,m,v){
		if(m.length>1){
			...
			if(!o[im]){
				var nim = m.slice(0,1).toString();
				if(nim=='' || nim==parseInt(nim)){
					o[im]=[];
				} else {
					o[im]={};
				}
