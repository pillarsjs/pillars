...
function Gangway(req,res){
	...
	gw.id = YMDHMS((new Date()),true)+'.'+req.socket.remoteAddress+'.'+Math.round(Math.random()*1000000000000000).toString(36);
	...
	gw.cache = {
		...
		nonematch: req.headers['if-none-match']?decrypt(req.headers['if-none-match']):false,
...
Gangway.prototype.head = function(){
	...
	if(Object.keys(gw.etag).length>0){
		gw.setHeader("Etag", encrypt(gw.etag));
	}
	...
	gw.setHeader("Server", 'Pillars.js v'+ENV.version);
...
function rangesParser(req){
	...
	return {
		check: req.headers['if-range']?decrypt(req.headers['if-range']):false,
		unit:unit,
		start:start,
		end:end
	};
