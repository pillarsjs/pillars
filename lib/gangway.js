...
Gangway.prototype.event = function(name,meta,callback){
  ...
  if(ons.length>0){
    ...
    for(i=0,l=ons.length;i<l;i++){
      var event = ons[i];
      procedure.add('gw.event.'+name+"."+event.handler.name,event.handler,meta);
    }
...
Gangway.prototype.file = function(file,clientname,download){
  ...
  function fileCheck(_stats){
    ...
    if(!gw.cacheck(gw.lastmod)) {
      stats = _stats;
      if(stats.size>pillars.config.maxZipSize){gw.encoding='identity';}
...
Gangway.prototype.error = function(code,error){
  ...
  if(util.isError(error)){
    ...
    gw.render(pillars.config.errorTemplate,{
      title:explain,
      h1:h1,
      error:error,
      stack:error.stack
