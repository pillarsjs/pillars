...
Gangway.prototype.setUser = function(_id,callback){
	...
	users.findOne({_id:_id},function(error, result) {
		if(!error && result){
			gw.user = result;
			if(typeof gw.user.keys === "string"){gw.user.allkeys = gw.user.keys.split(',');}
			gw.user.can = function(keys){
				if(!keys){return true;}
				if(isString(keys)){var keys = [keys];}
				if(isArray(keys)){
					if(keys.length==0){return true;}
					for(var i in keys){
						if(gw.user.keys.indexOf(keys[i])>=0){
							return true;
						}
					}
				}
...
Gangway.prototype.error = function(code,data){
	...
	if(util.isError(data)){
		gw.render(PILLARS.path('templates/error.jade'),{
			title:explain,
			h1:h1,
			error:data.toString(),
