...
function Textualization(){
	...
	Object.defineProperty(T12n,"langs",{
		...
		set : function(set){
			langs = set;
			refresh();
			console.log(t12n('textualization.langs',{langs:langs}));
		}
	});
	...
	function load(path,source){
		if(!source || source===path){
				...
				try {
					...
					if(cache[path][lang]){
						...
						for(var i in cache[path][lang]){
							if(heap[lang][i]){console.log(t12n('textualization.heap-rewrite',{path:path,lang:lang,element:i}));}
							heap[lang][i]=cache[path][lang][i];
						}
					}
					console.log(t12n('textualization.load-ok',{path:path,lang:lang,count:Object.keys(cache[path][lang]).length}));
				} catch(error){
					console.log(t12n('textualization.load-error',{path:path,lang:lang}),error.stack);
				}
			}
			cache[path]=path;
		} else {
			...
			for(var l in langs){
				...
				if(source[lang]){
					if(!heap[lang]){heap[lang]={};}
					if(heap[lang][path]){console.log(t12n('textualization.heap-rewrite',{path:path,lang:lang,element:i}));}
					heap[lang][path]=source[lang];
					console.log(t12n('textualization.load-ok',{path:path,lang:lang,count:Object.keys(source[lang]).length}));
				}
			}
			cache[path] = source;
		}
	...
	T12n.t12n = t12n;
	function t12n(text,params,lang) {
