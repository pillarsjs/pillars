...
function Gangway(req,res){
  ...
  gw.https = req.socket.server.ssl?true:false;
...
Gangway.prototype.file = function(file,clientname,download){
  ...
  if(typeof file === 'string'){
    stats = Gangway.cache.stats(path);
  ...
  function fileStream(cache){
    ...
    if(gw.ranges && (gw.ranges.check === false || gw.ranges.check.hash === stats.mtime.getTime())){
      ranges = true;
...
Gangway.prototype.json = function(data,decycledConfig){
  ...
  var gw = this;
  gw.send(JSON.decycled(data,decycledConfig),"application/json");
};
