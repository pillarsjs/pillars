...
var fs = require('fs');
var logger = global.logger.pillars.plugins.addGroup('BodyReader');
...
module.exports = new Plugin({id:'BodyReader',priority:1006},function(gw,next){
...
function urlEncoded(gw,callback){
	...
	gw.req.on('end', function () {
		if($error){
			...
			var params = queryHeap(querystring.parse(buffer, '&', '='));
			gw.content.params = params;
			for(var v in params){gw.params[v] = params[v];}
			callback();
		}
...
function multipart(gw,callback){
