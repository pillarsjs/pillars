...
function Project(){
	...
	function findRoutes(gw){
		var path = gw.method+'+'+'http://'+gw.host+gw.path;
		for(var pillar in project.pillars){
			//var path = project.pillars[p].getPath();
			var regexps = project.pillars[pillar].getRegexps();
			for(var beam in regexps){
				if(regexps[beam].test(path)){
					gw.beam = project.pillars[pillar].getBeam(beam);
					gw.pillar = gw.beam.getPillar();
					gw.pathMatchs = regexps[beam].exec(path).slice(1);
					readEntity(gw);
					return true;
				}
			}
