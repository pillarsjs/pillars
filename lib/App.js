...
function App(config){
	...
	if(config.languages){app.languages = config.languages;}
	...
	if(config.database){app.database = config.database;}
			...
			if(pillar.host.test(gw.host) && pillar.path.test(gw.path)){
				...
				for(var ib in beams){
					...
					if(beam.method.concat(['OPTIONS','HEAD']).indexOf(gw.method)>=0 && beam.path.test(beamPath)){
						...
						if(beam.cors){
							if(typeof beam.cors === 'string'){
								gw.cors.origin = beam.origin;
								gw.cors.methods = beam.method.concat(['OPTIONS','HEAD']);
							} else if(beam.cors.origin){
								gw.cors.origin = beam.cors.origin;
								if(beam.cors.methods){gw.cors.methods = beam.cors.methods;}
								if(beam.cors.headers){gw.cors.headers = beam.cors.headers;}
							}
						}
