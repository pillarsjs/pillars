...
var fs = require('fs');
var logger = global.logger.pillars.addGroup('renderer');
...
function Renderer(){
	...
	var engines = {};
	Object.defineProperty(renderer,"engines",{
		enumerable : true,
	...
	renderer.load = function(path){
		...
		if(engine){
			try {
				...
				logger.error('error',{path:path,error:error});
				return false;
			}
		} else {
	...
	renderer.render = function(path,locals){
		...
		} else if(ENV.templates.error && cache[ENV.templates.error]){
			locals.h1 = locals.gw.i18n('pillars.renderer.template-noexist',{path:path});
			return cache[ENV.templates.error](locals);
		} else {
			return locals.gw.i18n('pillars.renderer.template-noexist',{path:path});
		}
	}
}
