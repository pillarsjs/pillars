...
Schema.prototype.list = function(gw,params,cb){
	...
	if(typeof params._sort === 'string'){
		for(var header in schema.headers){
			if(schema.headers[header].id == params._sort){sort = params._sort;break;}
		}
	}
	...
	var cols = {};
	for(var header in schema.headers){cols[schema.headers[header].id]=true;}
