	...
	Schema.prototype.getter = function(doc,user,cb,params){
		...
		for(var fi in schema.fields){
			if(doc[fi]){
				...
				console.log(field.need('see',owner,guest));
				console.log(user.can(field.need('see',owner,guest)));
				//if(user.can(field.need('see',owner,guest))){
...
function Field(id,config){
	...
	field.keys = {
		...
		edit: config.keys.see || null,
		manage: config.keys.see || null
	};
}
