...
function Field(id,config){
	...
	Object.defineProperty(field,"label",{
		get : function(){
			return "schemas."+field.schema.id+".fields."+field.path+".label";
		}
	});

	Object.defineProperty(field,"details",{
		get : function(){
			return "schemas."+field.schema.id+".fields."+field.path+".details";
		}
	});
...
function List(id, config){
	var field = this;
	var config = config || {};
	config.template = config.template || 'list';
	Field.call(field, id, config);
	field.fields = {};
}
