...
function Fieldset(id,config){
	...
	Fieldset.prototype.getter = function(gw,id,data,cb){
		var fieldset = this;
		var data = data || {};
		var path = [id];
		...
		FieldWalker(path,fieldset.fieldsCan('see',gw.keys,true,false),'_getter',data,handler,end);
	};
	Fieldset.prototype.setter = function(gw,id,data,cb){
		var fieldset = this;
		var data = data || {};
		var path = [id];
		var result = new Group();
		...
		FieldWalker(path,fieldset.fieldsCan('edit',gw.keys,true,false),'_setter',data,handler,end);
	};
	...
	if(_id){
		...
		db.findOne({_id:_id},function(error, result) {
			if(error){
				...
				console.time("getter");
				fieldset.getter(gw,_id,result,function(getted,errors){
...
Fieldset.prototype.update = function(gw,params,cb){
	...
	if(_id && doc){
		fieldset.setter(gw,_id,doc,function(setted,errors){
			if(errors){
				cb({
