...
function routesWalker(gw,route,path){
	if(route.active && route.pathRegexp.test(path)){
		var match = path.match(route.pathRegexp);
		...
		if(isEndPath || haveChildren){
			if(isEndPath){
				if((!route.host || route.host==gw.host) && (!route.method || route.method.concat(['OPTIONS']).indexOf(gw.method)>=0) && (route.https===undefined || route.https===gw.https)){
